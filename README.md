# Consent2Share Master User Interface API

The Consent2Share Master User Interface API (master-ui-api) is a Backend For Frontends(BFF) component of Consent2Share (C2S)
...
+ To build a Docker Image (this will create an image with `bhitsdev/master-ui-api:latest` tag):
...
+ Run as a JAR file: `java -jar master-ui-api-x.x.x-SNAPSHOT.jar <additional program arguments>`
+ Run as a Docker Container: `docker run -d bhitsdev/master-ui-api:latest <additional program arguments>`
...
+ `java -jar master-ui-api-x.x.x-SNAPSHOT.jar --server.port=80 --spring.datasource.password=strongpassword`

#### Override a Configuration Using Program Arguments While Running as a Docker Container:

+ `docker run -d bhitsdev/master-ui-api:latest --server.port=80 --spring.datasource.password=strongpassword`
...
services:
...
  master-ui-api.master.com:
    image: "bhitsdev/master-ui-api:latest"
...
+ `java -jar master-ui-api-x.x.x-SNAPSHOT.jar --spring.profiles.active=ssl --server.ssl.key-store=/path/to/ssl_keystore.keystore --server.ssl.key-store-password=strongkeystorepassword`

#### Enable SSL While Running as a Docker Container

+ `docker run -d -v "/path/on/dockerhost/ssl_keystore.keystore:/path/to/ssl_keystore.keystore" bhitsdev/master-ui-api:latest --spring.profiles.active=ssl --server.ssl.key-store=/path/to/ssl_keystore.keystore --server.ssl.key-store-password=strongkeystorepassword`
+ In a `docker-compose.yml`, this can be provided as:
```yml
version: '2'
services:
...
Please use [GitHub Issues](https://github.com/bhits-dev/master-ui-api-api/issues) page to report issues.
